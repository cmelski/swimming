...
def add_results():
    ...
    if add_results_form.validate_on_submit():
        ...
        con.cursor.execute(f"SELECT event_id from event where stroke = '{event}';")
...
def get_comp_results():
    ...
    for i in range(0, len(results_data)):
        con.cursor.execute(f"SELECT stroke from event where event_id = '{results_data[i][2]}';")
